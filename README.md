...
also require the VCS you use, `jj` or `git`, on `PATH`. [Filters](#filters)
require [`jp`](https://github.com/jmespath/jp), and only when one is used.
...
A repository list is often generated by something that knows far more about
each repository than `herd` needs. Those extra fields are kept as they are,
and a **filter** selects repositories by them. A filter is a
[JMESPath](https://jmespath.org/) expression, evaluated by
[`jp`](https://github.com/jmespath/jp), which `herd` looks for on `PATH` the
first time a filter is used and never otherwise.

Filters are named in `:filters`, and everything else refers to them by name:

```janet
{:filters
 {"active"   "[?schedules[?starts_with(name, 'nightly-')]]"
  "platform" "[?group=='platform']"
  "reviewed" "[?approvals > `0`]"}

 :checkouts
 [{:from "repos.json" :anchor "src"     :filter ["active" "reviewed"]}
  {:from "repos.json" :anchor "archive" :filter []}]}
```

An expression is applied to the whole array in a list file, exactly as it is
written on disk, so what `jp` prints at a shell prompt is what `herd` sees:

```sh
jp "[?group=='platform']" <~/.config/herd/repos.json
```

Develop an expression that way, then name it. Naming is what the rest of
`herd` uses, which keeps JMESPath out of shell quoting — in
``[?approvals > `0`]`` the backticks that mark a JSON literal are command
substitution to the shell. A name nothing defines is an error rather than a
selection that quietly matches nothing.

A `:checkouts` row takes its filters in `:filter`, always as an array, and
they run as a pipeline: the first is applied to the list, the second to what
the first answered with, and so on, exactly as `jp A <list.json | jp B`
would. For the usual `[?…]` expressions that amounts to an intersection, and
the order makes no difference — `["active" "reviewed"]` selects the
repositories that are both. An expression that is not a filter does not
commute, though: `["active" "[0:1]"]` takes the first of the active
repositories, while `["[0:1]" "active"]` takes the first repository of the
list and keeps it only if it is active. A row replaces the `:defaults`
filters rather than adding to them, as it does for `:vcs` and `:anchor`, so
`:filter []` is how a row keeps everything when the defaults filter.

Because a row filters the list before anchoring it, two rows can take
different parts of one file, and only the entries a filter kept are read as
repositories — a list may carry entries that are not repositories at all.

Filters narrow what `herd` knows about rather than what one command does, so
`clone` checks out only what is left, and `list`, `fetch`, `run` and custom
commands all see the same repositories. On the command line, `-f` (or
`--filter`) names a filter to apply on top of whatever the rows already
filter, and repeating it narrows further:

```sh
herd list --filter active
herd clone -f active -f reviewed
herd run --filter platform -- jj st
```

A filter named this way is applied to **every** configured list, while a
row's `:filter` applies only to its own `:from`. Lists rarely have the same
shape — that is why filters exist — and a field a list does not carry reads
as null. Most JMESPath functions reject null rather than treating it as no
match, so an expression meant for `-f` has to name fields defensively:

```janet
{"platform" "[?group == 'platform' || starts_with(group || '', 'platform/')]"}
```
...
Selection has two parts, and both must hold. A [filter](#filters) narrows
the result further.
...
Use `-f NAME` (or `--filter NAME`) to keep only the repositories a
[named filter](#filters) selects, whatever their location. Repeating it
feeds each filter what the one before it answered with, on top of whatever
the rows already filtered:

```sh
herd list -a -C / --filter active
